...
class MapPiece extends BufferedImage{
				...
				if(robot.GetLocation().equals(point)) {
					...
					for(int i = 0 ; i < resize.getWidth() ;i++) {
						...
						for(int j = 0 ; j <resize.getHeight();j++) {
							...
							g.drawImage(resize, this.getWidth()/4, this.getHeight()/4, null);
	...
	static boolean checkMatchingColor(MapPiece mp, Robot robot) {

		if (mp.pieceType.startsWith("g") && robot.currentShape.equals("Hexagon")) {

			return true;
		}
		if (mp.pieceType.startsWith("r") && robot.currentShape.equals("Star")) {
			return true;
		}
		if (mp.pieceType.startsWith("b") && robot.currentShape.equals("Diamond")) {
			return true;
		}
		if (mp.pieceType.startsWith("y") && robot.currentShape.equals("Moon")) {
			return true;
